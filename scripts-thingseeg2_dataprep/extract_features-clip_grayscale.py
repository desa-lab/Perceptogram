...
images = np.load("data/thingseeg2_metadata/test_images_grayscale.npy", mmap_mode='r')
...
np.save('cache/thingseeg2_extracted_embeddings/test_clip_grayscale.npy', embeddings)

# Load the train_images NumPy array
images = np.load("data/thingseeg2_metadata/train_images_grayscale.npy", mmap_mode='r')
# Convert each image to a PIL image
images = [Image.fromarray(image).convert("RGB") for image in images]
...
os.makedirs('cache/thingseeg2_extracted_embeddings', exist_ok=True)
np.save('cache/thingseeg2_extracted_embeddings/train_clip_grayscale.npy', embeddings)