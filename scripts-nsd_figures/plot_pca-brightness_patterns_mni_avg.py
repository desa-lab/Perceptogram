...
vol_dir = f'cache/nsd_preproc/predicted_patterns/pca-brightness_patterns/sub-01/mni/'
vol_filenames = os.listdir(vol_dir)
pattern_names = [vol_filename.split('_')[0] for vol_filename in vol_filenames]

output_folder = f'results/nsd_preproc/avg-1-2-5-7/pca-brightness_patterns/mni/'
os.makedirs(output_folder, exist_ok=True)
os.makedirs('cache/nsd_preproc/predicted_patterns/pca-brightness_patterns/avg-1-2-5-7/mni/', exist_ok=True)

for i, vol_filename in enumerate(vol_filenames):
    subs = [1,2,5,7]
    volumes = []
    for sub in subs:
        vol_dir = f'cache/nsd_preproc/predicted_patterns/pca-brightness_patterns/sub-{sub:02d}/mni/'
    ...
    nib.save(pattern_image, f'cache/nsd_preproc/predicted_patterns/pca-brightness_patterns/avg-1-2-5-7/mni/{vol_filename}')
    ...
    volumn = cortex.Volume(np.moveaxis(np.moveaxis(avg_vol,0,-1),0,1), subject='fsaverage', xfmname='atlas', cmap='twilight', vmin=-1., vmax=1.)
    fig = plt.figure() # 100
