...
vol_dir = f'cache/nsd_preproc/predicted_patterns/vdvae-texture_patterns/sub-01/mni/'
vol_filenames = os.listdir(vol_dir)
pattern_names = [vol_filename.split('_')[0] for vol_filename in vol_filenames]

output_folder = f'results/nsd_preproc/avg-1-2-5-7/vdvae-texture_patterns/mni/'
os.makedirs(output_folder, exist_ok=True)
os.makedirs('cache/nsd_preproc/predicted_patterns/vdvae-texture_patterns/avg-1-2-5-7/mni/', exist_ok=True)
...
for i, vol_filename in enumerate(vol_filenames):
    ...
    nib.save(pattern_image, f'cache/nsd_preproc/predicted_patterns/vdvae-texture_patterns/avg-1-2-5-7/mni/{vol_filename}')
    volumn = cortex.Volume(np.moveaxis(np.moveaxis(avg_vol,0,-1),0,1), subject='fsaverage', xfmname='atlas', cmap='twilight', vmin=-0.4, vmax=0.4)
    fig = plt.figure() # 100
