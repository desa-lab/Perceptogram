...
# Load data
with open(f'cache/nsd_preproc/regression_weights/sub-{sub:02d}/regress-encode_ica1k_weights.pkl',"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight'].T
    reg_b = datadict['bias']
print(reg_w.shape, reg_b.shape)

train_latents= np.load(f'cache/nsd_extracted_embeddings/train_ica1k_sub-{sub:02d}.npy')
test_latents = np.load(f'cache/nsd_extracted_embeddings/test_ica1k.npy')
...
pred_latents = np.load(f'cache/nsd_preproc/predicted_embeddings/sub-{sub:02d}/regress_ica1k.npy')
...
train_mean = ica["mean"]
latent_dim = 1000
pred_images = np.clip(decoder[:, :latent_dim] @ pred_latents.T + train_mean[:, np.newaxis], 0, 1).T.reshape((len(pred_latents), 64, 64, 3), order="F")
...
map1 = new_w_pred_whitened[hue_argsort][100:300].mean(axis=0) # Red
map2 = new_w_pred_whitened[hue_argsort][500:700].mean(axis=0) # Green
map3 = new_w_pred_whitened[hue_argsort][700:900].mean(axis=0) # Blue
...
os.makedirs(f'results/nsd_preproc/sub-{sub:02d}/ica-color_patterns/func1pt8mm/', exist_ok=True)
os.makedirs(f'cache/nsd_preproc/predicted_patterns/ica-color_patterns/sub-{sub:02d}/func1pt8mm/', exist_ok=True)
maps = [map1, map2, map3]
pattern_names = ['red', 'green', 'blue']
for i_map, map in enumerate(maps):
    # plot subject-native pattern
    betas_trial_masked[mask<1] = 0
    betas_trial_masked[mask==1] = map
    vol_data = cortex.Volume(np.moveaxis(np.moveaxis(betas_trial.get_fdata(),0,-1),0,1), f'subj{sub:02d}', 'full', cmap='twilight', vmin=-0.2, vmax=0.2)
    ...
    plt.savefig(f'results/nsd_preproc/sub-{sub:02d}/ica-color_patterns/func1pt8mm/{pattern_names[i_map]}_pattern.png', dpi=300)    
    plt.close()

    pattern_image = nib.Nifti1Image(betas_trial_masked, affine=betas_trial.affine, header=betas_trial.header)
    nib.save(pattern_image, f'cache/nsd_preproc/predicted_patterns/ica-color_patterns/sub-{sub:02d}/func1pt8mm/{pattern_names[i_map]}_pattern.nii.gz')