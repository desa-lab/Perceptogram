...
for sub in tqdm(range(1, 11), total=10):
    ...
    for i_time in range(0, 60, 2):
        ...
        linear_interp = griddata(channel_locs, food_map[i_time], (grid_x2, grid_y2), method='linear')
        nan_mask = np.isnan(linear_interp)
        grid_z = griddata(channel_locs, food_map[i_time], (grid_x, grid_y), method='nearest')
        grid_z[nan_mask] = np.nan
        grid_z = np.ma.masked_where((grid_z == 0), grid_z)
        axs[row, column].imshow(grid_z.T, extent=(grid_x.min()- extension * x_range, grid_x.max()+ extension * x_range, grid_y.min()- extension * y_range, grid_y.max()+ 0.2 * y_range), origin='lower', cmap='RdBu_r', vmin=-2, vmax=2)
...
plt.tight_layout()
plt.savefig('results/thingseeg2_preproc/food_pattern.png')
...
for sub in tqdm(range(1, 11), total=10):
    ...
    animals_map = all_animals_maps[sub-1]
    ...
    for i_time in range(0, 60, 2):
        ...
        # Interpolate the EEG data over the grid
        linear_interp = griddata(channel_locs, animals_map[i_time], (grid_x2, grid_y2), method='linear')
        nan_mask = np.isnan(linear_interp)
        grid_z = griddata(channel_locs, animals_map[i_time], (grid_x, grid_y), method='nearest')
        grid_z[nan_mask] = np.nan
        grid_z = np.ma.masked_where((grid_z == 0), grid_z)
        axs[row, column].imshow(grid_z.T, extent=(grid_x.min()- extension * x_range, grid_x.max()+ extension * x_range, grid_y.min()- extension * y_range, grid_y.max()+ 0.2 * y_range), origin='lower', cmap='RdBu_r', vmin=-2, vmax=2)
...
plt.subplots_adjust(hspace=0.0)
plt.tight_layout()
plt.savefig('results/thingseeg2_preproc/animals_pattern.png')
...
for sub in tqdm(range(1, 11), total=10):
    ...
    others_map = all_others_maps[sub-1]
    ...
    for i_time in range(0, 60, 2):
        ...
        # Interpolate the EEG data over the grid
        linear_interp = griddata(channel_locs, others_map[i_time], (grid_x2, grid_y2), method='linear')
        nan_mask = np.isnan(linear_interp)
        grid_z = griddata(channel_locs, others_map[i_time], (grid_x, grid_y), method='nearest')
        grid_z[nan_mask] = np.nan
        grid_z = np.ma.masked_where((grid_z == 0), grid_z)
        axs[row, column].imshow(grid_z.T, extent=(grid_x.min()- extension * x_range, grid_x.max()+ extension * x_range, grid_y.min()- extension * y_range, grid_y.max()+ 0.2 * y_range), origin='lower', cmap='RdBu_r', vmin=-2, vmax=2)
...
plt.subplots_adjust(hspace=0.0)
plt.tight_layout()
plt.savefig('results/thingseeg2_preproc/others_pattern.png')