...
for sub in tqdm(range(1, 11), total=10):
    ...
    for i_time in range(0, 60, 2):
        ...
        linear_interp = griddata(channel_locs, blue_map[i_time], (grid_x2, grid_y2), method='linear')
        nan_mask = np.isnan(linear_interp)
        grid_z = griddata(channel_locs, blue_map[i_time], (grid_x, grid_y), method='nearest')
        grid_z[nan_mask] = np.nan
        grid_z = np.ma.masked_where((grid_z == 0), grid_z)
        axs[row, column].imshow(grid_z.T, extent=(grid_x.min()- extension * x_range, grid_x.max()+ extension * x_range, grid_y.min()- extension * y_range, grid_y.max()+ 0.2 * y_range), origin='lower', cmap='RdBu_r', vmin=-0.02, vmax=0.02)
...
plt.tight_layout()
plt.savefig('results/thingseeg2_preproc/blue_pattern.png')
...
for sub in tqdm(range(1, 11), total=10):
    ...
    for i_time in range(0, 60, 2):
        ...
        # Interpolate the EEG data over the grid
        linear_interp = griddata(channel_locs, red_map[i_time], (grid_x2, grid_y2), method='linear')
        nan_mask = np.isnan(linear_interp)
        grid_z = griddata(channel_locs, red_map[i_time], (grid_x, grid_y), method='nearest')
        grid_z[nan_mask] = np.nan
        grid_z = np.ma.masked_where((grid_z == 0), grid_z)
        axs[row, column].imshow(grid_z.T, extent=(grid_x.min()- extension * x_range, grid_x.max()+ extension * x_range, grid_y.min()- extension * y_range, grid_y.max()+ 0.2 * y_range), origin='lower', cmap='RdBu_r', vmin=-0.02, vmax=0.02) # 0.8 for alpha=1000

plt.subplots_adjust(hspace=0.0)
plt.tight_layout()
plt.savefig('results/thingseeg2_preproc/red_pattern.png')