...
images = np.load("data/nsd_metadata/test_images.npy", mmap_mode='r').astype(np.uint8)
...
os.makedirs('cache/nsd_extracted_embeddings', exist_ok=True)
np.save('cache/nsd_extracted_embeddings/test_clip.npy', embeddings)

for sub in [1,2,5,7]:
    # Load the train_images NumPy array
    images = np.load(f'data/nsd_metadata/train_images_sub-{sub:02d}.npy', mmap_mode='r').astype(np.uint8)
    # Convert each image to a PIL image
    images = [Image.fromarray(image).convert("RGB") for image in images]
    ...
    os.makedirs('cache/nsd_extracted_embeddings', exist_ok=True)
    np.save(f'cache/nsd_extracted_embeddings/train_clip_sub-{sub:02d}.npy', embeddings)