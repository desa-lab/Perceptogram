...
stim_order_f = 'data/nsd_metadata/experiments/nsd/nsd_expdesign.mat'
...
roi_dir = 'data/nsd_preproc/subj{:02d}/func1pt8mm/roi/'.format(sub)
betas_dir = 'data/nsd_preproc/subj{:02d}/func1pt8mm/betas_fithrf_GLMdenoise_RR/'.format(sub)
...
f_stim = h5py.File('data/nsd_metadata/stimuli/nsd/nsd_stimuli.hdf5', 'r')
...
os.makedirs(f'data/nsd_preproc/sub-{sub:02d}',exist_ok=True)
np.save(f'data/nsd_preproc/sub-{sub:02d}/train_fmriavg_nsdgeneral.npy',fmri_array )
np.save(f'data/nsd_metadata/train_images_sub-{sub:02d}.npy',stim_array )
...
np.save(f'data/nsd_preproc/sub-{sub:02d}/test_fmriavg_nsdgeneral.npy',fmri_array)
np.save(f'data/nsd_metadata/test_images.npy',stim_array)

print("Test data is saved.")

annots_cur = np.load('data/nsd_metadata/COCO_73k_annots_curated.npy')

captions_array = np.empty((num_train,5),dtype=annots_cur.dtype)
for i,idx in enumerate(train_im_idx):
    captions_array[i,:] = annots_cur[idx,:]
    print(i)
np.save(f'data/nsd_metadata/train_texts_sub-{sub:02d}.npy',captions_array)
    
captions_array = np.empty((num_test,5),dtype=annots_cur.dtype)
for i,idx in enumerate(test_im_idx):
    captions_array[i,:] = annots_cur[idx,:]
    print(i)
np.save(f'data/nsd_metadata/test_texts.npy',captions_array)
