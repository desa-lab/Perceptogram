...
for i, image in tqdm(enumerate(test_data)):
    test_latents[i] = eigenvectors[:latent_dim] @ image.reshape(64, 64, 3).T.flatten()
np.save("cache/nsd_extracted_embeddings/test_pca1k.npy", test_latents)

train_latents = np.zeros((len(train_data), latent_dim))
for i, image in tqdm(enumerate(train_data)):
    train_latents[i] = eigenvectors[:latent_dim] @ image.reshape(64, 64, 3).T.flatten()
np.save(f"cache/nsd_extracted_embeddings/train_pca1k_sub-{sub:02d}.npy", train_latents)