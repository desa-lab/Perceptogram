...
norm_mean_test = np.mean(eeg_test, axis=0)
norm_scale_test = np.std(eeg_test, axis=0, ddof=1)
eeg_train = (eeg_train - norm_mean_train) / norm_scale_train
eeg_test = (eeg_test - norm_mean_test) / norm_scale_test
...
weights_filename = f'regress-for-saliency-encode_clip_weights{param}.pkl'
# weights_filename = f'regress-encode_clip_weights{param}_grayscale.pkl'
save_dir = f'cache/thingseeg2_preproc/predicted_embeddings/sub-{sub:02d}/'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
pred_filename = f'regress-for-saliency-encode_clip{param}.npy'
# pred_filename = f'regress-encode_clip{param}_grayscale.npy'
...
# test_latents = (test_latents - test_latents_mean) / test_latents_std
