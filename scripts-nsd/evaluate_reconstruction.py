...
images_dir = f'results/nsd_preproc/sub-{sub:02d}/unclip{param}'
feats_dir = f'cache/nsd_preproc/eval_features/sub-{sub:02d}/unclip{param}/'

if not using_cliptext and not using_clipvision:
    images_dir = f'results/nsd_preproc/sub-{sub:02d}/vdvae'
    feats_dir = f'cache/nsd_preproc/eval_features/sub-{sub:02d}/vdvae/'
...
num_test = 200
test_dir = f'cache/nsd_test_images_eval_features'

feats_dir = f'cache/nsd_preproc/eval_features/sub-{sub:02d}/unclip{param}'
gen_image_dir = f'results/nsd_preproc/sub-{sub:02d}/unclip{param}/'
performances_save_dir = f'results/nsd_preproc/sub-{sub:02d}/performances_unclip{param}.npy'

# if you want to evaluate VDVAE only
if not using_cliptext and not using_clipvision:
    feats_dir = f'cache/nsd_preproc/eval_features/sub-{sub:02d}/vdvae'
    gen_image_dir = f'results/nsd_preproc/sub-{sub:02d}/vdvae/'
    performances_save_dir = f'results/nsd_preproc/sub-{sub:02d}/performances_vdvae.npy'
...
for i in range(200):
    gt_image = Image.open(f'data/nsd_metadata/test_images_direct/{i}.png').resize((512,512)) # either both 512 or both 500
    gen_image = Image.open(gen_image_dir + '{}.png'.format(i)).resize((512,512))
